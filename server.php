...
$link = mysqli_connect('localhost', 'id9002852_admin', 'veralaras', 'id9002852_kopi');
...
if (strcmp($data, 'data') ==0) {
 ...
 case 'GET':
    ...
    echo "select * from pelangan ";break;
    }
    else
    {
         $sql = "select * from pelangan where id='$id'";
         echo "select * from pelangan where id='$id'";break;
...
if ($method == 'POST') {
        ...
        $idplg=$json["id"];
        $namaplg=$json["nama"];
        $alamatplg=$json["alamat"];
		$querycek = "SELECT id,nama,alamat FROM pelangan WHERE id ='$idplg'";
